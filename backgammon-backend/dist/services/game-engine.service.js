...
exports.gameEngineService = exports.GameEngineService = void 0;
const random_utils_1 = require("../utils/random.utils");
class GameEngineService {
    /**
     * Create initial board setup - standard backgammon position
     */
    initializeGame(firstPlayer = 'white') {
        const board = new Array(24).fill(null).map(() => ({
            pieces: 0,
            color: null,
        }));
     ...
     */
    rollForFirst() {
        const white = random_utils_1.randomUtils.rollDie();
        const black = random_utils_1.randomUtils.rollDie();
        let first = 'tie';
        if (white > black)
            first = 'white';
        else if (black > white)
     ...
     */
    rollDice() {
        const die1 = random_utils_1.randomUtils.rollDie();
        const die2 = random_utils_1.randomUtils.rollDie();
     ...
     */
    getBarEntryMoves(gameState, color, dieValues) {
        const moves = [];
    ...
    canBearOff(gameState, color) {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (point.color === color && point.pieces > 0) {
                if (color === 'white' && i < 18)
                    return false; // White piece outside home
                if (color === 'black' && i > 5)
                    return false; // Black piece outside home
            }
        }
        return true;
    }
     ...
     */
    isValidBearOff(gameState, color, from, dieValue) {
    ...
    getGameResult(gameState) {
        if (!this.isGameOver(gameState))
            return null;
        const winner = gameState.off.white === 15 ? 'white' : 'black';
     ...
     */
    hasPieceInOpponentHome(gameState, loser, winner) {
        // Winner's home board
...
exports.gameEngineService = new GameEngineService();
